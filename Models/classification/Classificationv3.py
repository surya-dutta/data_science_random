...
ROOT_DIR = "../../Common_Utils/wade/"

spec = importlib.util.spec_from_file_location(
    "wade.balanced", f"{ROOT_DIR}./Balance.py"
)
# spec = importlib.util.spec_from_file_location('wade.balanced', '../../utils/wade/Balance.py')
...
def evaluate(self, y_hat_class, Y): # Y = real value, Y_hat = expected
    cm = np.array([[0, 0], [0, 0]])
    if not (y_hat_class == Y.flatten()).all():
        return evaluator(Y, y_hat_class)
    cm[0][0] += sum(y_hat_class == 0)
    cm[1][1] += sum(y_hat_class == 1)

def evaluator(self, Y, y_hat_class):
    ...
    p0 = t0 / (t0 + f0) if (t0 + f0) != 0.0 else 0.0
    p1 = t1 / (t1 + f1) if (t1 + f1) != 0.0 else 0.0
    r = t0 / (t0 + f1) if (t0 + f1) != 0.0 else 0.0
    fscore0 = 2 * p0 * r / (p0 + r) if p0 + r != 0.0 else 0.0
    ...
    return np.array([wa, r, pavg, f1avg, a, s, p0, p1, fscore0, fscore1, d, j]), cm
...
def split_data(target, data, class_var, test_frac=0.2, transfer=False, balance=False):
    ...
    if transfer == True:
        X_train, X_test, Y_train, Y_test = train_test_split(
            X.to_numpy(), Y.to_numpy(), test_size=test_frac, shuffle=True
        )
        ...
        if balance == True:
            ...
            train_dataset = wade.BalanceClassesWithDuplicates(
                train_dataset, class_var, printDebug=False
            )
...
def run_cv(
    ...
    for i in range(3):
        ...
        if clfnm == "DecisionTree":
            ...
            Grid_DT_depth = GridSearchCV(
                sklearn.tree.DecisionTreeClassifier(), parameters, cv=5
            )
        ...
        if clfnm == "K-NN":
            ...
            GridKNN = GridSearchCV(
                sklearn.neighbors.KNeighborsClassifier(), param_grid=parameters, cv=5
            )
        ...
        if clfnm == "Logit Regression":
            ...
            GridLR = GridSearchCV(
                sklearn.linear_model.LogisticRegression(), param_grid=parameters, cv=5
            )
...
def save_Draw_Roc(roc_features, save_path=False, printDebug=True):
    ...
    for i in range(len(roc_features)):
        ...
        roc_y = sklearn.preprocessing.label_binarize(
            roc_features[i][0], classes=[0, 1, 2]
        )
        roc_y_pred = sklearn.preprocessing.label_binarize(
            roc_features[i][1], classes=[0, 1, 2]
        )
...
def save_Draw_FeatureImportanceMatrix(
    dft, feature_importances, classifierName, save_path=False, printDebug=True
):
    ...
    for i in range(len(feature_importances)):
        ...
        sorted_idx10 = sorted_idx[-m:]
        # [0:5]  # TODO: Print Top 10 only ??
        pos = np.arange(sorted_idx10.shape[0]) + 0.5
        fc10 = np.asanyarray(dft.columns.tolist())[sorted_idx10]

        axs[i].barh(
            pos, feature_importance[sorted_idx10], align="center", color="#7A68A6"
        )
