...
### so here we get the orginal data and it has to be split to 80-20 ratio Train(80) and Test(20)
### Now the test n=data is untouched and data augumentation (balncing) is done on 80% train data 
def split_data(data, class_var, minority_var, test_frac=0.2, balance=False):
    ...
    for train_index, test_index in kf.split(X.to_numpy()):
        ...
        if balance == True:
            ...
            train_dataset = balance_method.generate_autoencoder(train_dataset, class_var=class_var, minority_var=minority_var, printDebug=False)
